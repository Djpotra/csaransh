...
def unsupervisedClustersClassification(feat):
    rndSeed = 42
    reduced_dim = umap.UMAP(n_components=8, n_neighbors=9, min_dist=0.11,
...
def clusterClasses(data):
    ...
    try:
        ...
        print(show_dim[:100])
    try:
        print('##########try UMAP + HDBSCAN classification##########')
        ...
        print("Continuing with non-fatal error in supervised classification")
        print(sys.exc_info()[0])
    print('##########try savi classification##########')
    compLabels = []
    cnt = 0
    for tcas, tcid in tag:
        if data[tcas]['clusterSizes'][tcid] > 1000 :
            # print('savi cluster ',cnt,' in ',len(tag)," too big , aborted");
            continue
        # else :
            # print('savi cluster ',cnt,' in ',len(tag))
        cnt = cnt + 1
