...
def findComponentClass(attrs, labels, components, lines, ringFreeIs):
    ...
    if onlyParallel:
        ...
        if alone:
            if ringFreeIs > 0:
                if allParSizes[0][0] < 20: return (0, "a", "||@") # TODO check type of ring
            orientV = allParSizes[0][1]['verdict']
            if orientV == 3: return (0, "a", "||")
            if orientV == 1: return (0, "a","||-!")
            return (0, "a", "#")
        return (0, "a", "||//")
    ...
    #print(centComps)
    if ringFreeIs > 0 and attrs['nMain'] < 3: return (0, "a", "@")
    if centComps[1] > 0.4:
        #print("cent ring:", centComps[0], centComps[1], sum(centComps[2:]))
        if centComps[1] < 0.75 and centComps[0] > sum(centComps[2:]): return (0, "a", "@||")
        if centComps[1] < 0.75 and sum(sizesAny[2:]) >= 3: return (0, "a", "@#")
        return (0, "a","@")
    ...
    if centComps[0] > 0.4 and allParSizes[0][0] > 2:
        ...
        if stillParallel:
            if centComps[2] > 0.5 and sizesMain[2] > 0: return (0, "a","||#")
            if sizesMain[0] > 1: return (0, "a", "||//")
            orient = allParSizes[0][1]['verdict']
            if orient == 1: return (0, "a","||-!")
            if orient == 3: return (0, "a","||")        
            return (0, "a","||#")            
        if (sizesAny[1] > 0) and centComps[1] <= 0.2:  return (0, "a","||#")
    if (centComps[0] < 0.2 or allParSizes[0][0] <= 2) and sizesMain[1] == 0 and ringFreeIs == 0: return (0, "a","#")
    if sizesAny[1] > 0 or ringFreeIs > 0: return (0, "a","#@")
    if sizesMain[0] == 1:  return (0, "a","||#")
    if sizesMain[0] > 1: return (0, "a","||//")
    return (0, "a", "?")
