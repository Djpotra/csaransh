...
csaransh::DefectVecT csaransh::groupDefects(const csaransh::DefectVecT &defects,
  ...
  for (const auto &it : defects) {
    std::cout << "grouping defect "<< cnt++ <<" in "<<defects.size()<<'\r';
    uf.uniteIf(it, pred);
  }
...
void csaransh::ignoreSmallClusters(csaransh::DefectVecT &defects,
  ...
  // int cnt = 1;
  // for(auto &it: defects) {
  //   std::cout<<cnt++<<": "<<get<0>(it)[0] << ", " << get<0>(it)[1] << ", " << get<0>(it)[2] << ", " << get<1>(it) << ", " << get<2>(it) << ", " << get<3>(it) <<std::endl;
  // }
}
...
csaransh::clusterFeatures(const csaransh::DefectVecT &defects,
  ...
  for (const auto &it : clusters) {
    if(it.second.size()>1000){
      std::cout << "find cluster feature  "<< cnt++ <<" in "<<clusters.size()<<"too big! aborted"<<std::endl;
      continue;
    }
    std::cout << "find cluster feature  "<< cnt++ <<" in "<<clusters.size()<<'\r';
