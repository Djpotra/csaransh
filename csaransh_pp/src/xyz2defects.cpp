...
getAtomsTime(csaransh::InputInfo &info, csaransh::ExtraInfo &extraInfo,
  ...
  std::pair<vector<Coords>,vector<Coords>>
      originAtomsTemp;
  std::pair<vector<Coords>,vector<Coords>>
      originAtoms;
  ...
  csaransh::lineStatus ls;
  res.first = csaransh::xyzFileStatus::eof;
  // std::cout<<"Opening originfile:"<<info.originFilePath<<"\n";
  std::ifstream originfile{info.originFilePath};
  if (!originfile.bad() && originfile.is_open()) {
    std::cout<<"Working with originfile: "<<info.originFilePath<<"\n";
    csaransh::frameStatus ofs = fs;
    int flag = 1;
    while (std::getline(originfile, line)) {
      std::tie(ls, c) = csaransh::getCoord(line, ofs, info, extraInfo);
      if (ls == csaransh::lineStatus::coords &&
          ofs == csaransh::frameStatus::inFrame) {
        if(flag){
          originAtomsTemp.first.emplace_back(c);
        }else{
          originAtomsTemp.second.emplace_back(c);
        }
        flag = !flag;
        ...
        if (ofs != csaransh::frameStatus::prelude) {
          ofs = csaransh::frameStatus::inFrame;
          originAtomsTemp.first.clear();
          originAtomsTemp.second.clear();
        }
        ofs = csaransh::frameStatus::inFrame;
      }
    }
    std::cout<<"originfile read done\n";
    // std::cout<<info.ncell<<" "<<info.sizeY<<" "<<info.sizeX<<"\n";
    if(originAtomsTemp.first.size()>0){
      int timeX = info.ncell / info.sizeX;
      int timeY = info.ncell / info.sizeY;
      int partSize = info.ncell * info.sizeY * info.sizeX;

      for(int z=0;z<info.ncell;z++){
        for(int q=0;q<timeY;q++){
          for(int y=0;y<info.sizeY;y++){
            for(int p=0;p<timeX;p++){      
              for(int x=0;x<info.sizeX;x++){
                int offset = (q + p*timeY)*partSize + z*info.sizeY*info.sizeX + y*info.sizeX + x;
                // std::cout<<originAtomsTemp.first[offset][0]<<" ";
                // std::cout<<originAtomsTemp.first[offset][1]<<" ";
                // std::cout<<originAtomsTemp.first[offset][2]<<"\n";
                originAtoms.first.emplace_back(originAtomsTemp.first[offset]);
              }
            }
          }
        }
    ...
    if(originAtomsTemp.second.size()>0){
      int timeX = info.ncell / info.sizeX;
      int timeY = info.ncell / info.sizeY;
      int partSize = info.ncell * info.sizeY * info.sizeX;
        ...
        for(int q=0;q<timeY;q++){
          for(int y=0;y<info.sizeY;y++){
            for(int p=0;p<timeX;p++){      
              for(int x=0;x<info.sizeX;x++){
                int offset = (q + p*timeY)*partSize + z*info.sizeY*info.sizeX + y*info.sizeX + x;
                originAtoms.second.emplace_back(originAtomsTemp.second[offset]);
              }
            }
          }
        }
    ...
    std::cout<<"Open originfile failed! File name recevied: "<<info.originFilePath<<"\n";
  }
  ...
  if(atoms.empty()){
    csaransh::Logger::inst().log_debug("Atoms read error!");
  }
  ...
  if(!originfile.bad() && originfile.is_open()){
    auto obj = csaransh::myAddOffset{info.latticeConst, info.ncell,info.structure, origin,originAtoms};
...
csaransh::DefectRes csaransh::atoms2defects(
  ...
  for (const auto &row : atoms) {
    ...
    // std::cout<<ar[0]<<" "<<ar[1]<<" "<<ar[2]<<" || "<<nextExpected.cur()[0]<<" "<<nextExpected.cur()[1]<<" "<<nextExpected.cur()[2]<<" "<<"\n";
  ...
  */
  ...
  for (auto i = 0; i < vacancies.size(); i++) {
    ...
    // std::cout<<"vacancies: "<<get<0>(it)[0] << ", " << get<0>(it)[1] << ", " << get<0>(it)[2] << "\n";
    //std::cout << "I is " << i << ": ";
    for (auto j = (i > 0) ? vacSias[i-1] : 0; j < vacSias[i]; j++) {
      //std::cout << j << ", ";
      auto &jt = interstitials[j];
      // defects.emplace_back(anchor2coord(std::move(get<1>(jt))), true, count++, get<2>(jt));
      defects.emplace_back(std::move(get<1>(jt)), true, count++, get<2>(jt));
      // std::cout<<"interstitials: "<<get<1>(jt)[0] << ", " << get<1>(jt)[1] << ", " << get<1>(jt)[2] << "\n";
  ...
  // int cnt = 1;
  // for(auto &it: defects) {
  //   std::cout<<cnt++<<": "<<get<0>(it)[0] << ", " << get<0>(it)[1] << ", " << get<0>(it)[2] << ", " << get<1>(it) << ", " << get<2>(it) << ", " << get<3>(it) << '\n';
  // }
  ...
  //   cnt += csaransh::DefectTWrap::isSurviving(it) ? 1 : 0;
  // }
  // std::cout<<"size :"<<defects.size()<<"  "<<"surviving :"<<cnt<<std::endl;
